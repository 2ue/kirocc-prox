...
func (m *AuthManager) refreshCredentials(ctx context.Context) (*Credentials, error) {
	...
	if err := m.writeToDB(refreshed); err != nil {
...
func (m *AuthManager) writeToDB(creds *Credentials) error {
	...
	if _, err := db.Exec(`INSERT INTO auth_kv(key, value) VALUES(?, ?)
		ON CONFLICT(key) DO UPDATE SET value = excluded.value`, key, string(tokenJSON)); err != nil {
		return fmt.Errorf("write token: %w", err)
	}
	if creds.ProfileARN != "" {
		if _, err := db.Exec(`INSERT INTO state(key, value) VALUES(?, ?)
			ON CONFLICT(key) DO UPDATE SET value = excluded.value`, "api.codewhisperer.profile", creds.ProfileARN); err != nil {
