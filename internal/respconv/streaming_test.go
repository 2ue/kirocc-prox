...
func TestSSEWriter_ThinkingWithLateSignature(t *testing.T) {
	w := httptest.NewRecorder()
	sw := NewSSEWriter(context.Background(), w, "claude-sonnet-4.6", 200000, nil, 0, 0)

	sw.HandleEvent(kiroproto.Event{Type: "reasoningContentEvent", ThinkingText: "Let me"})
	sw.HandleEvent(kiroproto.Event{Type: "reasoningContentEvent", ThinkingText: "Let me think"})
	sw.HandleEvent(kiroproto.Event{Type: "reasoningContentEvent", Signature: "sig_late"})
	sw.HandleEvent(kiroproto.Event{Type: "assistantResponseEvent", Content: "Answer"})
	sw.Finish()
	...
	if !strings.Contains(body, `"signature":"sig_late"`) {
		t.Fatal("missing late signature")
	}
	if strings.Contains(body, `"type":"thinking","thinking":"","signature":"sig_late"`) {
		t.Fatal("late signature should be emitted as signature_delta, not block start")
	}
}
...
func TestSSEWriter_InvalidToolUseDroppedSilently(t *testing.T) {
	// The proxy must never synthesize assistant-visible text describing
	// dropped tool_use; Claude Code would treat it as model output. Invalid
	// calls are exposed only via InvalidToolCalls() for retry orchestration.
	w := httptest.NewRecorder()
	sw := NewSSEWriter(context.Background(), w, "claude-opus-4.7", 1000000, nil, 0, 0)
	sw.SetToolInputValidator(NewToolInputValidator([]anthropic.Tool{{
		Name: "Edit",
	...
	sw.HandleEvent(kiroproto.Event{Type: "toolUseEvent", ToolStop: true, ToolUseID: "toolu_bad", ToolName: "Edit", ToolInput: `{}`})
	calls := sw.InvalidToolCalls()
	sw.Finish()

	body := w.Body.String()
	if strings.Contains(body, `"type":"tool_use"`) {
		t.Fatalf("invalid tool_use should not be emitted, body: %s", body)
	}
	if strings.Contains(body, "invalid tool call") || strings.Contains(body, "Upstream produced") {
		t.Fatalf("warning text must not bleed into stream, body: %s", body)
	}
	if len(calls) != 1 || calls[0].Name != "Edit" {
		t.Fatalf("InvalidToolCalls should expose dropped call, got %#v", calls)
	}
	if !strings.Contains(body, `"stop_reason":"end_turn"`) {
		t.Fatalf("expected end_turn, body: %s", body)
	}
}

func TestSSEWriter_MixedValidAndInvalidToolUse_NoWarningLeak(t *testing.T) {
	// Real-world hot path: model emits one valid tool_use first (gate
	// promoted, retry impossible) and a second invalid tool_use. The valid
	// call must be forwarded; the invalid one is silently dropped from the
	// stream and the proxy must NOT append a "⚠ ..." text block afterwards.
	w := httptest.NewRecorder()
	sw := NewSSEWriter(context.Background(), w, "claude-opus-4.7", 1000000, nil, 0, 0)
	sw.SetToolInputValidator(NewToolInputValidator([]anthropic.Tool{
		{Name: "Bash", InputSchema: map[string]any{"type": "object", "required": []any{"command"}}},
		{Name: "Edit", InputSchema: map[string]any{"type": "object", "required": []any{"file_path", "old_string", "new_string"}}},
	}))

	sw.HandleEvent(kiroproto.Event{Type: "toolUseEvent", ToolStop: true, ToolUseID: "toolu_ok", ToolName: "Bash", ToolInput: `{"command":"ls"}`})
	sw.HandleEvent(kiroproto.Event{Type: "toolUseEvent", ToolStop: true, ToolUseID: "toolu_bad", ToolName: "Edit", ToolInput: `{}`})
	calls := sw.InvalidToolCalls()
	sw.Finish()
	...
	body := w.Body.String()
	if !strings.Contains(body, `"name":"Bash"`) {
		t.Fatalf("valid tool_use missing from stream: %s", body)
	}
	if strings.Contains(body, `"name":"Edit"`) {
		t.Fatalf("invalid tool_use must not be emitted: %s", body)
	}
	if strings.Contains(body, "invalid tool call") || strings.Contains(body, "Upstream produced") {
		t.Fatalf("warning text leaked into mixed-call stream: %s", body)
	}
	if len(calls) != 1 || calls[0].Name != "Edit" {
		t.Fatalf("InvalidToolCalls should still expose dropped call, got %#v", calls)
	}
}
