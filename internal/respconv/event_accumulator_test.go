...
func TestAccumulator_InvalidToolUseDropped(t *testing.T) {
	acc := newAccumulator(200000, nil, 0, 0)
	acc.toolInputValidator = NewToolInputValidator([]anthropic.Tool{{
		Name: "Edit",
	...
	d := acc.ProcessEvent(kiroproto.Event{
		Type: "toolUseEvent", ToolStop: true,
		ToolUseID: "toolu_bad", ToolName: "Edit", ToolInput: `{}`,
	})
	if d.ToolStop {
		t.Fatal("invalid tool_use should not be forwarded")
	}
	if acc.HasToolUse {
		t.Fatal("invalid tool_use should not set HasToolUse")
	}
	if len(acc.ToolCalls) != 0 {
		t.Fatalf("ToolCalls len = %d, want 0", len(acc.ToolCalls))
	}
	if len(acc.InvalidToolCalls) != 1 {
		t.Fatalf("InvalidToolCalls len = %d, want 1", len(acc.InvalidToolCalls))
	}
	if acc.InvalidToolCalls[0].Name != "Edit" {
		t.Fatalf("invalid name = %q", acc.InvalidToolCalls[0].Name)
	}
}

func TestAccumulator_ValidToolUseNormalizedEmptyObject(t *testing.T) {
	acc := newAccumulator(200000, nil, 0, 0)
	acc.toolInputValidator = NewToolInputValidator([]anthropic.Tool{{Name: "Ping"}})
	d := acc.ProcessEvent(kiroproto.Event{
		Type: "toolUseEvent", ToolStop: true,
		ToolUseID: "toolu_ok", ToolName: "Ping", ToolInput: "",
	})
	if !d.ToolStop {
		t.Fatal("valid tool_use should be forwarded")
	}
	if d.ToolInput != "{}" {
		t.Fatalf("ToolInput = %q, want {}", d.ToolInput)
	}
	if len(acc.ToolCalls) != 1 || acc.ToolCalls[0].Input != "{}" {
		t.Fatalf("ToolCalls = %+v", acc.ToolCalls)
	}
}

func TestAccumulator_InvalidToolUseValidatorUsesMappedToolName(t *testing.T) {
	acc := newAccumulator(200000, nil, 0, 0)
	acc.toolNameMap = map[string]string{"short_edit": "Edit"}
	acc.toolInputValidator = NewToolInputValidator([]anthropic.Tool{{
		Name: "Edit",
		InputSchema: map[string]any{
			"type":     "object",
	...
	d := acc.ProcessEvent(kiroproto.Event{
		Type: "toolUseEvent", ToolStop: true,
		ToolUseID: "toolu_bad", ToolName: "short_edit", ToolInput: `{}`,
	})
	if d.ToolStop {
		t.Fatal("invalid mapped tool_use should not be forwarded")
	}
	if acc.HasToolUse || len(acc.ToolCalls) != 0 {
		t.Fatalf("invalid mapped tool_use should not be recorded: has=%v calls=%d", acc.HasToolUse, len(acc.ToolCalls))
	}
	if len(acc.InvalidToolCalls) != 1 {
		t.Fatalf("InvalidToolCalls len = %d, want 1", len(acc.InvalidToolCalls))
	}
	if acc.InvalidToolCalls[0].Name != "Edit" {
		t.Fatalf("invalid tool name = %q, want mapped original Edit", acc.InvalidToolCalls[0].Name)
	}
}
