...
// [fork] Added in fork: InvalidToolCall type and the InvalidToolCalls slice on
// responseAccumulator below (fix #5). Captures tool_use blocks that the input
// validator (tool_input_validator.go) rejected so they can be reflowed to the
// model as tool_result(is_error=true) instead of leaking to Claude Code.
//
// InvalidToolCall records a model-emitted tool_use that was unsafe to forward
// to the client because its JSON input did not satisfy the advertised schema.
...
type responseAccumulator struct {
	...
	// toolInputValidator validates model-emitted tool input before forwarding it.
