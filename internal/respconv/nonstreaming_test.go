...
func TestNonStreamingAccumulator_InvalidToolUseDroppedSilently(t *testing.T) {
	// Mirror of streaming behavior: invalid tool_use is dropped, no warning
	// text is synthesized into the response body. Retry path uses
	// InvalidToolCalls() instead.
	acc := NewNonStreamingAccumulator(1000000, nil, 0, 0)
	acc.SetToolInputValidator(NewToolInputValidator([]anthropic.Tool{{
		Name: "Write",
	...
	acc.ProcessEvent(kiroproto.Event{Type: "toolUseEvent", ToolStop: true, ToolUseID: "toolu_bad", ToolName: "Write", ToolInput: `{}`})

	calls := acc.InvalidToolCalls()
	resp, _ := acc.BuildResponse("claude-opus-4.7")

	if len(calls) != 1 || calls[0].Name != "Write" {
		t.Fatalf("InvalidToolCalls should expose dropped call before BuildResponse, got %#v", calls)
	}
	if resp["stop_reason"] != "end_turn" {
		t.Fatalf("stop_reason = %v, want end_turn", resp["stop_reason"])
	}
	...
	for _, raw := range content {
		...
		if block["type"] == "text" {
			...
			if strings.Contains(text, "invalid tool call") || strings.Contains(text, "Upstream produced") {
				t.Fatalf("warning text leaked into response body: %q", text)
			}
		}
		if block["type"] == "tool_use" && block["name"] == "Write" {
			t.Fatalf("invalid tool_use should not be in response body: %#v", block)
		}
	}
}
