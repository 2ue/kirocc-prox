...
func (c *HTTPClient) GenerateAssistantResponse(ctx context.Context, token string, payload *kiroproto.Payload, region string) (*Response, error) {
	...
	auditPayload(ctx, payload, body, promptTokens)
...
type auditPayloadRecord struct {
	...
	HistorySamples  []auditHistoryEntry `json:"history_samples"`
...
type StartupAuditInfo struct {
	GitSHA             string            `json:"git_sha"`
	BinaryPath         string            `json:"binary_path"`
	ThinkingPrefixMode string            `json:"thinking_prefix_mode"`
	Env                map[string]string `json:"env,omitempty"`
}

type auditStartupRecord struct {
	Time       string `json:"time"`
	RecordType string `json:"record_type"`
	StartupAuditInfo
}

func AuditStartup(info StartupAuditInfo) {
	path := os.Getenv("KIROCC_AUDIT_LOG")
	if path == "" {
		return
	}
	rec := auditStartupRecord{
		Time:             time.Now().Format(time.RFC3339Nano),
		RecordType:       "startup",
		StartupAuditInfo: info,
	}
	b, err := json.Marshal(rec)
	if err != nil {
		slog.Warn("audit startup marshal failed", "err", err)
		return
	}
	if err := appendAuditLine(path, b); err != nil {
		slog.Warn("audit startup open failed", "path", path, "err", err)
	}
}

func auditPayload(ctx context.Context, payload *kiroproto.Payload, body []byte, promptTokens int) {
	path := os.Getenv("KIROCC_AUDIT_LOG")
	if path == "" || payload == nil {
		return
	}
	...
	ctxMsg := cur.UserInputMessageContext
	var toolNames []string
	var toolResults int
	if ctxMsg != nil {
		toolResults = len(ctxMsg.ToolResults)
		for _, tool := range ctxMsg.Tools {
			if tool.ToolSpecification != nil {
				toolNames = append(toolNames, tool.ToolSpecification.Name)
			}
		}
	}

	history := payload.ConversationState.History
	samples := make([]auditHistoryEntry, 0, min(len(history), 8))
	for _, h := range history {
		if len(samples) >= 8 {
			break
		}
		if h.UserInputMessage != nil {
			content := h.UserInputMessage.Content
			samples = append(samples, auditHistoryEntry{Role: "user", Len: len([]rune(content)), Prefix: prefixRunes(content, 1800)})
			continue
		}
		if h.AssistantResponseMessage != nil {
			content := h.AssistantResponseMessage.Content
			samples = append(samples, auditHistoryEntry{Role: "assistant", Len: len([]rune(content)), Prefix: prefixRunes(content, 1200)})
		}
	}

	all := string(body)
	rec := auditPayloadRecord{
		Time:            time.Now().Format(time.RFC3339Nano),
		...
		CurrentLen:      len([]rune(cur.Content)),
		CurrentPrefix:   prefixRunes(cur.Content, 5000),
		HistoryCount:    len(history),
		HistorySamples:  samples,
		ToolCount:       len(toolNames),
		ToolNames:       toolNames,
		ToolResultCount: toolResults,
		DetectedMarkers: detectAuditMarkers(all),
	}

	b, err := json.Marshal(rec)
	if err != nil {
		slog.WarnContext(ctx, "audit payload marshal failed", "err", err)
		return
	}
	if err := appendAuditLine(path, b); err != nil {
		slog.WarnContext(ctx, "audit payload open failed", "path", path, "err", err)
	}
}

func appendAuditLine(path string, b []byte) error {
...
func detectAuditMarkers(s string) []string {
	checks := []string{
		"Claude Agent SDK",
		"Claude Code",
		"AmazonQ-For-CLI",
