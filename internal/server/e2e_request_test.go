...
func TestE2E_ThinkingMode_XMLInjection(t *testing.T) {
	// Without effort=max/xhigh and without KIROCC_FORCE_THINKING_BUDGET, the
	// proxy must NOT inject thinking_mode XML; that prefix biases adaptive
	// thinking and pollutes routine turns. See plan P0-B.
	t.Run("default_no_injection", func(t *testing.T) {
		...
		if strings.Contains(content, "<thinking_mode>") || strings.Contains(content, "<max_thinking_length>") {
			t.Fatalf("default request should not inject thinking_mode prefix: %q", content)
		}
		...
		if ctx != nil {
			for _, te := range ctx.Tools {
				if te.ToolSpecification != nil && te.ToolSpecification.Name == "thinking" {
					t.Fatal("thinking tool should not be present (prompt-injection mode is gated)")
				}
			}
		}
	})

	t.Run("effort_max_injects", func(t *testing.T) {
		p1 := mustJSON(map[string]string{"content": "result"})
		client := &capturingClient{events: []any{"assistantResponseEvent", p1}}
		...
		resp := postMessages(t, srv.URL, `{"model":"claude-sonnet-4-6[1m]","messages":[{"role":"user","content":"think about this"}],"stream":false,"output_config":{"effort":"max"}}`)
		defer func() { _ = resp.Body.Close() }()
		...
		if !strings.Contains(content, "<thinking_mode>enabled</thinking_mode>") {
			t.Fatalf("effort=max should inject thinking_mode: %q", content)
		}
		if !strings.Contains(content, "think about this") {
			t.Fatalf("should contain original user text: %q", content)
		}
	...
	t.Run("force_budget_floor_injects", func(t *testing.T) {
		...
		content := client.captured.ConversationState.CurrentMessage.UserInputMessage.Content
		if !strings.Contains(content, "<thinking_mode>enabled</thinking_mode>") {
			t.Fatalf("KIROCC_FORCE_THINKING_BUDGET should inject thinking_mode: %q", content)
		}
		if !strings.Contains(content, "<max_thinking_length>100000</max_thinking_length>") {
			t.Fatalf("budget floor should drive max_thinking_length: %q", content)
		}
	})
}
