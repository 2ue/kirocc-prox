...
func (o *toolSearchOrchestrator) handleStreaming(ctx context.Context, w http.ResponseWriter) retryOutcome {
	...
	// terminate the whole request with HTTP 400 (the upstream behavior).
	// Mirrored in handleNonStreaming below.
	...
	for round := range maxToolSearchRounds {
		...
		if !foundToolSearch {
			if calls := sw.InvalidToolCalls(); len(calls) > 0 && !gw.IsPromoted() {
				if invalidToolRetried {
					gw.Discard()
					slog.ErrorContext(ctx, "tool search invalid tool retry still returned invalid tool_use",
						"trace_id", short,
						"round", round+1,
		...
		if parseErr != nil {
			...
			searchInput = fallbackSearchInput(toolSearchInput)
		}
...
func (o *toolSearchOrchestrator) handleNonStreaming(ctx context.Context, w http.ResponseWriter) retryOutcome {
	...
	for round := range maxToolSearchRounds {
		...
		if parseErr != nil {
			slog.WarnContext(ctx, "tool search input parse error", "trace_id", short, "err", parseErr)
			searchInput = fallbackSearchInput(nsToolSearchInput)
		}
		var results []string
		searchErr := parseErr
		if searchErr == nil {
...
func fallbackSearchInput(input string) map[string]any {
	...
	if rawMax, ok := parsed["max_results"].(float64); ok {
		if maxResults := clampToolSearchMaxResults(int(rawMax)); maxResults > 0 {
...
func parseToolSearchInput(input string) (query string, maxResults int, err error) {
	...
	if uerr := json.Unmarshal([]byte(input), &parsed); uerr != nil {
		return "", 0, fmt.Errorf("%w: parse tool_search input: %v", toolsearch.ErrInvalidInput, uerr)
	}
	query = strings.TrimSpace(parsed.Query)
	if query == "" {
		return "", 0, fmt.Errorf("%w: missing query", toolsearch.ErrInvalidInput)
	}
	return query, clampToolSearchMaxResults(int(parsed.MaxResults)), nil
}

func clampToolSearchMaxResults(maxResults int) int {
	if maxResults <= 0 {
		return 0
	}
	return min(maxResults, toolsearch.MaxResults)
}
