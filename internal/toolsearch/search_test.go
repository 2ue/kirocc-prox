...
func TestSearchClampsMaxResults(t *testing.T) {
	...
	for i := range 30 {
		...
		tools[name] = anthropic.Tool{Name: name, Description: "matching tool"}
	}
	got, err := Search("matching", tools, SearchTypeRegex, 1000)
	if err != nil {
		t.Fatal(err)
	}
	if len(got) > MaxResults {
		t.Fatalf("got %d results, want at most %d", len(got), MaxResults)
	}
}
